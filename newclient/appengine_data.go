...
// Parses data obtained by performing a request to add a device alias.
...
func (r AddDeviceAliasResponse) Raw() *http.Response {
	return r.res
}
...
func (d *DatastreamPaginator) parseData(rawData []byte) any {
	...
	return parseDatastream([]byte(data), "")
}

func parseDatastream(rawData []byte, prefix string) any {
	// Base case: an array of values (individual aggregation, no parameters, complete path given)
	if gjson.ParseBytes(rawData).IsArray() && gjson.GetBytes(rawData, "value").Exists() {
		data := gjson.ParseBytes(rawData).Array()
		ret := []DatastreamIndividualValue{}
		for _, v := range data {
		...
		return ret
	} else if gjson.ParseBytes(rawData).IsArray() {
		//if we're here, we have an object aggregation:
		// Strong assumption: no endpoint ends in "value"
		// Example:
		// 	[
		// 		{
		// 		  "foo": 1,
...
func (d *DatastreamPaginator) computePageState(rawData []byte) {
	...
	if resultLength < d.pageSize {
		...
		d.hasNextPage = true
		// TODO check this: it might also be an Object
		val := DatastreamIndividualValue{}
		_ = json.Unmarshal([]byte(data[resultLength-1].Raw), &val)
		// val must now be the most recent timestamp for the new page
		d.windowNewerTimestamp = val.Timestamp
	}
}
